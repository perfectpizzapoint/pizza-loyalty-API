...
async function calculateMdHeatmap() {
  const btn = document.getElementById('btnCalcMdHeatmap');
  btn.disabled = true;
  btn.innerHTML = '<span class="spinner"></span> Calculating…';

  try {
    const data = await api({ action: 'getMonthDayHeatmap', type: mdHeatmapType });
    renderMdHeatmap(data.matrix);
    toast('Month-Day heatmap calculated!', 'success');
  } catch (e) {
...
async function handleSaveEntry() {
  ...
  try {
    const result = await api({ action: 'addEntry', mobile, amount, date, time, message, cashAmt: payAmts.cashAmt, upiAmt: payAmts.upiAmt, cardAmt: payAmts.cardAmt });
    if (result.error) {
...
function incrementDishVariant(variantKey, variantName, price, delta) {
  ...
  if (!order[variantKey]) {
    order[variantKey] = { name: variantName, price, qty: 0 };
  }
